...
for month in range(1, 13):
    monthFlights = fixedFlights[fixedFlights['month'] == month]
    flightCounts.append(len(monthFlights))
    depDelayAvs.append(pd.Series(monthFlights['dep_delay'].values).describe()['mean'])
corr = np.corrcoef(depDelayAvs, flightCounts)[0, 1]
b1 = pd.Series(depDelayAvs).describe()['mean'] - corr * pd.Series(flightCounts).describe()['mean'] / (pd.Series(flightCounts).describe()['std'] ** 4)
print('Correlation coefficient = ', corr)
plt.scatter(flightCounts, depDelayAvs)
plt.plot(flightCounts, b1 + corr * np.array(flightCounts) / (pd.Series(flightCounts).describe()['std'] ** 4))
plt.xlabel("Flight counts")
plt.ylabel("Average delay")
plt.savefig('D:\\MFK\\correlation.jpg')
plt.clf()
print('Regression equation:', corr / (pd.Series(flightCounts).describe()['std'] ** 4), '* x +', b1)
